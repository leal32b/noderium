...
describe("hideMarkers", () => {
  describe("update", () => {
    it("rebuilds decorations when doc changes", async () => {
      const { sut } = makeSut();
    ...
    it("rebuilds decorations when selection sets", async () => {
      const { sut } = makeSut();
      const view = createEditorView("# First heading\n# Second heading", [sut]);
      const plugin = getPlugin(view, sut);
      const originalDecorations = plugin.decorations;
    ...
    it("rebuilds decorations when viewport changes", async () => {
      const { sut } = makeSut();
      const view = createEditorView("# First heading\n# Second heading", [sut]);
      const plugin = getPlugin(view, sut);
      const originalDecorations = plugin.decorations;
      const fakeUpdate = { viewportChanged: true, view };
    ...
    it("does not rebuild decorations when nothing relevant changed", async () => {
      const { sut } = makeSut();
      const view = createEditorView("# First heading\n# Second heading", [sut]);
      const plugin = getPlugin(view, sut);
      const originalDecorations = plugin.decorations;
  ...
  describe("buildDecorations", () => {
    describe("HeaderMarkers", () => {
      it("hides HeaderMarkers # on lines without the cursor", async () => {
        const { sut } = makeSut();
    ...
    describe("EmphasisMarkers", () => {
      it("hides EmphasisMark (*) on lines without the cursor", async () => {
        const { sut } = makeSut();
      ...
      it("hides EmphasisMark (_) on lines without the cursor", async () => {
        const { sut } = makeSut();
      ...
      it("hides EmphasisMark (__) on lines without the cursor", async () => {
        const { sut } = makeSut();
    ...
    describe("StrikethroughMarkers", () => {
      it("hides StrikethroughMarkers ~~ on lines without the cursor", async () => {
        const { sut } = makeSut();
    ...
    describe("LinkMarkers", () => {
      it("hides LinkMarkers [](link) on lines without the cursor", async () => {
        const { sut } = makeSut();
    ...
    describe("ListMarkers", () => {
      it("hides ListMarkers (-) on lines without the cursor", async () => {
        const { sut } = makeSut();
    ...
    describe("TaskMarkers", () => {
      it("hides TaskMarkers [ ] on lines without the cursor", async () => {
        const { sut } = makeSut();
    ...
    describe("QuoteMarkers", () => {
      it("hides QuoteMarkers > on lines without the cursor", async () => {
        const { sut } = makeSut();
    ...
    describe("CodeMarkers", () => {
      it("hides CodeMarkers ` on lines without the cursor", async () => {
        const { sut } = makeSut();
    ...
    describe("WikilinkMarkers", () => {
      it("hides WikilinkMarkers [[Wikilink]] on lines without the cursor", async () => {
        const { sut } = makeSut();
      ...
      it("does not hide URL/LinkMark nodes when they are inside a wikilink", async () => {
        const { sut } = makeSut();
    ...
    describe("mergeRanges and edge cases", () => {
      it("handles empty document without throwing", async () => {
        const { sut } = makeSut();
      ...
      it("handles document with no markdown markers (plain text only)", async () => {
        const { sut } = makeSut();
      ...
      it("merges overlapping or adjacent ranges without RangeSetBuilder errors", async () => {
        const { sut } = makeSut();
      ...
      it("sorts ranges by from then by to (same from triggers secondary sort)", async () => {
        const { sut } = makeSut();
